...
export default function ChatPage() {
  ...
  useEffect(() => {
    if (!lastEvent) return;
    if (lastEvent.event === "typing") setIsTyping(Boolean((lastEvent.data as any)?.isTyping));
    if (lastEvent.event === "rag_context") setActiveRagContexts(((lastEvent.data as any)?.contexts as RagContext[]) ?? []);
    if (lastEvent.event === "ai_status") setAiStatus(lastEvent.data as unknown as AIStatusInfo);
    if (lastEvent.event === "message") {
      const msg = lastEvent.data as unknown as Message;
      addMessage(lastEvent.conversationId ?? activeConvId, {
        ...msg,
