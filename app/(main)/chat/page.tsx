...
export default function ChatPage() {
  ...
  useEffect(() => {
    if (!lastEvent) return;
    if (lastEvent.event === "typing") setIsTyping(Boolean(lastEvent.data.isTyping));
    if (lastEvent.event === "rag_context") setActiveRagContexts((lastEvent.data.contexts as RagContext[]) ?? []);
    if (lastEvent.event === "ai_status") setAiStatus(lastEvent.data as AIStatusInfo);
